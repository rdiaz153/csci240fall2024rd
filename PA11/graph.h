//Modified by Ricardo Diaz for PA11, Adjacency List Graph 11/22/24
...
class AdjacencyListGraph {
  private:
    ...
    typedef std::list< std::pair<ActualVertex*,ActualEdge*> > IncidenceList;
    // IncidenceMap is now IncidenceList, changed the map ADT to list and combined parameters into a pair.
  ...
  public:
    ...
    class Vertex {
        ...
        // friend vertexhash removed and ALG is now friend
      ...
      public:
        ...
        friend AdjacencyListGraph;
        // friend vertexhash got repeated here? so just added ALG as friend again
    };  //---------- end of Vertex class ---------
    ...
    class Edge {
        //friend graph is now friend ALG, edgehash commented out
        friend AdjacencyListGraph;
        // friend EdgeHash;
    ...
    Edge insert_edge(Vertex u, Vertex v, int weight = 1, E elem = E()) {
        if (u.vert->outgoing.count(v.vert) == 0) {   // new edge
            auto iter = edge_list.emplace(edge_list.end(), u.vert, v.vert, weight, elem);
            ...
            // RD 
            u.vert->outgoing.emplace_back(v.vert, &*iter);
            //unsure what line below does
...
//template <typename V, typename E>
//using VertexSet = std::unordered_set<typename AdjacencyListGraph<V,E>::Vertex, typename Graph<V,E>::VertexHash>;
...
// fix num_vertices, num_edges, vertices, degree, incident_edges, opposite, weight
template <typename V, typename E>
